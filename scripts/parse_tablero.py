...
def parse_summary_sheet(ws):
    ...
    while row <= ws.max_row:
        ...
        if col_a is None and col_b in ("TERRITORIO", "SUBGERENCIA", "AGENCIAS", "JEFATURA"):
            section = col_b
            if section == "AGENCIAS":
                break  # nivel persona/agencia: fuera de alcance v1
            row += 1
            continue
        ...
        if section == "TERRITORIO" and col_a:
            ...
            territorio_name = TERRITORIO_SHORT_NAME_MAP.get(
                current_territorio_short, current_territorio_short
            )
            parent = territorios_by_short_name.get(territorio_name)
            node = {
                "code": str(col_a).strip(),
